import type { StatePropertyAccessor, TurnContext } from "botbuilder";
import type {
  DialogState,
  DialogTurnResult,
...
import type {
  ...
  FlightFromEntity,
  FlightToEntity,
} from "@kcnt-bot/recognizer/flight-recognizer";
...
export class MainDialog extends ComponentDialog {
  ...
  async introStep(
    ...
    const weekLaterDate = "week later";
    const messageText = stepContext.options.restartMsg
      ? (stepContext.options.restartMsg as string)
    ...
    switch (LuisRecognizer.topIntent(luisResult)) {
      case "BookFlight": {
        ...
        const bookingDetails = {
          destination: toEntities.airport,
          origin: fromEntities.airport,
          travelDate: this.luisRecognizer.getTravelDate(luisResult),
        };
  ...
  async showWarningForUnsupportedCities(
    context: TurnContext,
    fromEntities: FlightFromEntity,
    toEntities: FlightToEntity
  ): Promise<void> {
  ...
  async finalStep(
    ...
    if (stepContext.result) {
      ...
      const msg = `I have you booked to ${result.destination} from ${result.origin} on ${result.travelDate}.`;
      await stepContext.context.sendActivity(
        msg,
        msg,
        InputHints.IgnoringInput
